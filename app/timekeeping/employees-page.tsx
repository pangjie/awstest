...
export default function EmployeesPage({titleTarget,isAdmin,openRecords,openScan,sortState,setSortState}:{titleTarget:HTMLDivElement|null;isAdmin:boolean;openRecords?:((badge:string)=>void);openScan?:((badge:string)=>void);sortState:EmployeeSortState;setSortState:Dispatch<SetStateAction<EmployeeSortState>>}){
  ...
  return <div className="time-page time-employees-page">
    {titleTarget&&createPortal(<div className="time-employee-titlebar"><div className="time-employee-title-filters"><label><span>搜索</span><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="员工 ID、姓名、组织或职能" aria-label="搜索员工"/></label><label><span>状态</span><select value={status} onChange={event=>setStatus(event.target.value)} aria-label="员工状态"><option value="active">在职</option><option value="inactive">已停用</option><option value="all">全部</option></select></label><p>共 {rows.length} 名员工</p></div><div className="time-employee-title-actions"><button className="time-secondary" onClick={()=>setAttendanceExportOpen(true)}>考勤导出</button>{isAdmin&&<button className="time-secondary" disabled={exporting||!data?.employees.length} onClick={()=>void exportScanAssets()}>{exporting?"正在生成…":"导出扫描素材"}</button>}<button className="primary" onClick={()=>setAdding(true)}>新增员工</button></div></div>,titleTarget)}
...
function AttendanceExportModal({close}:{close:()=>void}){
  ...
  return <div className="modal-backdrop"><form className="modal-card time-employee-modal" onSubmit={submit}><div className="modal-head"><div><span className="modal-kicker">员工数据</span><h3>考勤导出</h3></div><button type="button" onClick={close}>×</button></div><div className="time-edit-fields"><label><span>自然月</span><input type="month" value={period} onChange={event=>setPeriod(event.target.value)} required/></label><label><span>导出范围</span><select value={scope} onChange={event=>setScope(event.target.value as AttendanceExportScope)}><option value="month">整个自然月</option><option value="first-half">上半月（1–15日）</option><option value="second-half">下半月（16日–月末）</option></select></label>{error&&<div className="time-error">{error}</div>}</div><div className="modal-actions"><button type="button" onClick={close}>取消</button><button className="primary" disabled={pending}>{pending?"正在导出…":"导出 Excel"}</button></div></form></div>;
}
