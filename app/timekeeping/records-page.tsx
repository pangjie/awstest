...
export default function RecordsPage({titleTarget,isAdmin,initialBadge}:{titleTarget:HTMLDivElement|null;isAdmin:boolean;initialBadge:string}){
  ...
  return <div className="time-page time-records-page">
    {titleTarget&&createPortal(<div className="time-record-titlebar time-no-print"><form onSubmit={submit}><div className="time-record-lookup-input"><span aria-hidden="true">⌁</span><input ref={inputRef} type="password" value={badge} onChange={event=>{attemptedBadge.current="";setBadge(sanitizeEmployeeId(event.target.value))}} maxLength={8} placeholder="扫描员工卡" aria-label="扫描员工卡" autoComplete="off"/><b aria-live="polite">{pending?"识别中":"自动识别"}</b></div></form></div>,titleTarget)}
