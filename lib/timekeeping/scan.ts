...
async function findLeadWave(tx:TimeTransaction,employeeId:number,exceptId:number){return (await tx.select({id:timeWorkItems.id,waveNo:timeWorkItems.waveNo}).from(timeWaveAssignments).innerJoin(timeWorkItems,eq(timeWaveAssignments.workItemId,timeWorkItems.id)).where(and(eq(timeWaveAssignments.employeeId,employeeId),eq(timeWaveAssignments.role,"lead"),eq(timeWorkItems.status,"active"),ne(timeWorkItems.id,exceptId))).limit(1))[0]??null}
