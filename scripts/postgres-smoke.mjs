...
const fixedPaused=await request("/api/v1/timekeeping/scan",{method:"POST",body:{code:"ACT-OUT",employeeId:identified.payload.employeeId,requestId:scanId("fixed-out"),terminalId:"e2e"}});
assert.equal(fixedPaused.payload.event,"CLOCK_OUT");
assert.equal(fixedPaused.payload.snapshot.state,"off");
assert.equal(fixedPaused.payload.snapshot.currentProject.code,"TODO");
assert.match(fixedPaused.payload.message,/计时已暂停/);
const fixedResumed=await request("/api/v1/timekeeping/scan",{method:"POST",body:{code:"ACT-CLOCKIN",employeeId:identified.payload.employeeId,requestId:scanId("fixed-resume"),terminalId:"e2e-resume"}});
assert.equal(fixedResumed.payload.event,"CLOCK_IN_PROJECT_RESUME");
assert.equal(fixedResumed.payload.snapshot.state,"working");
assert.equal(fixedResumed.payload.snapshot.currentProject.code,"TODO");
...
const helperClockedOut=await request("/api/v1/timekeeping/scan",{method:"POST",body:{code:"ACT-OUT",employeeId:helperIdentified.payload.employeeId,requestId:scanId("helper-out"),terminalId:"e2e-helper"}});
...
assert.match(completedWave.payload.message,/已完结，计时已结束/);
const clockedOut=await request("/api/v1/timekeeping/scan",{method:"POST",body:{code:"ACT-OUT",employeeId:identified.payload.employeeId,requestId:scanId("out"),terminalId:"e2e-final-out"}});
...
const cardScanReport=await request(`/api/v1/timekeeping/card-scan?badge=${timeBadge}`);
assert.equal(cardScanReport.payload.snapshot.employee.badgeCode,timeBadge);
assert.ok(Array.isArray(cardScanReport.payload.attendanceEvents));
assert.ok(Array.isArray(cardScanReport.payload.projects));
assert.equal("period" in cardScanReport.payload,false);
assert.equal("days" in cardScanReport.payload,false);
