...
5. 合并到 `main` 后，由 `production` environment 控制部署。
...
4. Actions 调用 SSM `AWS-RunShellScript`，不通过 SSH 登录 EC2。
5. EC2 从 Secrets Manager 读取 RDS credential，从 ECR 拉取镜像。
6. EC2 停止旧容器、启动新容器并轮询 `/health/ready`。
7. 新版本未在 60 秒内就绪时，部署脚本恢复旧镜像并让工作流失败。
