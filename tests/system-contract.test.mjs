...
test("keeps the fresh timekeeping system isolated behind five page permissions",async()=>{
  ...
  assert.match(cardScanPage,/const startCamera[\s\S]*setReport\(null\);setSelection\(null\);setError\(""\);setNotice\(""\);[\s\S]*window\.isSecureContext/);
  assert.match(cardScanPage,/setReport\(null\);setSelection\(null\);setNotice/);
  assert.match(cardScanPage,/SOUND_PREFERENCE_KEY="time-card-scan-sound"/);
  assert.match(cardScanPage,/useState\(readSoundPreference\)/);
  assert.match(cardScanPage,/localStorage\.getItem\(SOUND_PREFERENCE_KEY\)!=="off"/);
  assert.match(cardScanPage,/if\(soundEnabled\)await prepareAudio\(\)/);
  ...
  assert.match(timekeepingCss,/\.time-card-scan-sound \{[^}]*position: absolute;[^}]*min-width: 92px;/s);
  assert.match(timekeepingCss,/\.time-card-scan-identity\.success \{[^}]*border-color: #55bd91;/s);
  ...
  assert.match(dashboardPage,/item\.status!=="completed"[\s\S]*totals\.orders\+=item\.orderCount;[\s\S]*totals\.pieces\+=item\.pieceCount/);
  assert.match(dashboardPage,/type==="多件"[\s\S]*totals\.multipleOrders\+=item\.orderCount/);
  ...
  assert.match(app,/useState<DashboardSortState>\(\{key:"default",descending:false\}\)/);
  assert.match(app,/dashboardSort=\{timeDashboardSort\} setDashboardSort=\{setTimeDashboardSort\}/);
  assert.match(await read("app/timekeeping/timekeeping-module.tsx"),/sortState=\{dashboardSort\} setSortState=\{setDashboardSort\}/);
  ...
  assert.match(dashboardPage,/setSortState\(\{key:"default",descending:false\}\)/);
  assert.match(dashboardPage,/label="波次工时" field="duration"/);
  assert.match(dashboardPage,/label="时效" field="efficiency"/);
  assert.doesNotMatch(dashboardPage,/每小时件数/);
  assert.match(dashboardPage,/sortWaves\(visibleProjects,sortState\)/);
  assert.match(dashboardPage,/left\.sortOrder-right\.sortOrder\|\|left\.id-right\.id/);
  assert.match(timekeepingCss,/\.time-dashboard-wave-table \.time-wave-sort \{[^}]*grid-template-columns: minmax\(0,auto\) 7px;/s);
  ...
  assert.match(dashboardPage,/const warehouseTask=dailyTasks\.find\(task=>task\.code==="WAREHOUSE"\)/);
  assert.match(dashboardPage,/<Metric label="波次工时"[\s\S]*\{warehouseTask&&<DailyTaskMetric task=\{warehouseTask\} openScan=\{openScan\}\/\>\}/);
  assert.match(dashboardPage,/<DailyTaskSummary tasks=\{remainingDailyTasks\} openScan=\{openScan\}\/\>/);
  ...
  assert.match(dashboardPage,/orderedProjects\.map\(waveExportRow\)/);
